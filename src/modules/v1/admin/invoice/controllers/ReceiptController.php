...
use app\modules\v1\admin\invoice\models\form\ReceiptForm;
...
class ReceiptController extends Controller
{
    ...
    public function actionUpdate()
    {
        ...
        if ($request->isPost) {
            ...
            $id = $request->post('id');
            if (!empty($id)) {
                $item = ReceiptForm::find()->where(['id' => $id])->one();
                if (!empty($item)) {
                    $item->load($data);
                    if ($item->validate() && $item->save()) {
    ...
    public function actionDelete()
    {
        ...
        if ($request->isPost) {
            ...
            $id = $data['id'];
            if (!empty($id)) {
                $item = Receipt::find()->where(['id' => $id])->one();
                if (!empty($item)) {
                    $item->status = Receipt::STATUS_DELETED;
                    $item->save(false);
                    return ResponseBuilder::json(true, $item, "UPDATE SUCCESS! ");
                }
                return ResponseBuilder::json(false, $item->getErrors(), "DATA EMPTY! ");
